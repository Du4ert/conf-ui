...
<div class="card mt-5 col-md-5">
    ...
    <div class="card-body">
        @isset($thesis)
            <div id="#thesis-download" class="file-download">
                <form action="{{ route('thesis.delete', $thesis->id) }}" method="post">
                    @csrf

                    <a class="btn  text-primary" href="{{ route('thesis.download', $thesis->id) }}"><i
                            class="far fa-file-alt me-1"></i>{{ $user->first_name . '-thesis' . '.' . pathinfo($thesis->file, PATHINFO_EXTENSION) }}</a>
                    @method('DELETE')
                    <button type="submit" class="btn btn-link text-danger"><i class="fa fa-trash"></i></button>
                </form>
            </div>
        @else
            <div class="file-upload" id="thesis-upload">
                <form action="{{ route('thesis.store') }}" method="POST" enctype="multipart/form-data" class="mt-2"
                    id="file-upload">
                    @csrf
                    ...
                    <div class="row mb-3">
                        <label for="title" class="col-md-4 col-form-label text-md-end"
                            required>{{ __('auth.thesis_name') }}</label>
                        <div class="col-md-6">
        ...
        $.ajax({
            ...
            error: function(response) {
                ...
                $.each(response.responseJSON.errors, function(key, value) {
                    // console.log(value);
                    $('#file-upload').find(".print-error-msg").find("ul").append('<li>' +
                        value + '</li>');
                });
            }
        });
