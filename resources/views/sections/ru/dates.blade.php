<section id="dates-section" class="dates-section section conf-bg-white">
        <div class="container">
        <h3 class="section-heading text-center mb-5 pb-lg-5">Важные даты</h3>

            <div class="row">
              <div class="col-lg-12">
          
                <div class="horizontal-timeline">
          
                  <ul class="list-inline items">
                    <li class="list-inline-item items-list">
                      <div class="px-4">
                        <div class="event-date badge bg-info">15 августа</div>
                        <h5 class="pt-2">Регистрация</h5>
                        <p class="text-muted">Последний день принятия регистрационной формы. </p>
                      </div>
                    </li>
                    <li class="list-inline-item items-list">
                      <div class="px-4">
                        <div class="event-date badge bg-success">1 августа</div>
                        <h5 class="pt-2">Ранняя оплата</h5>
                        <p class="text-muted">Последний день ранней оплаты регистрационного взноса. 
                        </p>
                      </div>
                    </li>
                    <li class="list-inline-item items-list">
                      <div class="px-4">
                        <div class="event-date badge bg-danger">20 августа</div>
                        <h5 class="pt-2">Тезисы</h5>
                        <p class="text-muted">Последний день приема тезисов для публикации.</p>
                      </div>
                    </li>
                    <li class="list-inline-item items-list">
                      <div class="px-4">
                        <div class="event-date badge bg-warning">7 октября</div>
                        <h5 class="pt-2">Начало</h5>
                        <p class="text-muted">Начало конференции.</br>
                            Welcomeparty!
                        </p>
                      </div>
                    </li>
                  </ul>
          
                </div>
          
              </div>
            </div>


    </div><!--//container-->
</section><!--//schedule-section-->
